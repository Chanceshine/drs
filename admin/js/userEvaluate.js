		...
		var options = {
			...
			$.each(json, function(index, item){
				if (json[index].reply ==null) {
					dataHtml += '<tr><td>'+ json[index].nid +'</td><td>'+json[index].rscore+'</td><td class="text">'+json[index].revaluate+'</td><td>'+json[index].sscore+'</td><td class="text">'+json[index].sevaluate+'</td><td title="'+json[index].posttime+'" class="text">'+json[index].posttime+'</td><td>暂无</td><td><button data-index="'+json[index].nid+'" class="btn btn-success noreply">回复</button></td>';
				}else {
					dataHtml += '<tr><td>'+ json[index].nid +'</td><td>'+json[index].rscore+'</td><td class="text">'+json[index].revaluate+'</td><td>'+json[index].sscore+'</td><td class="text">'+json[index].sevaluate+'</td><td title="'+json[index].posttime+'" class="text">'+json[index].posttime+'</td><td>'+json[index].replyer+'</td><td><button data-index="'+json[index].nid+'" class="btn btn-success reply" title="点击查看回复内容">已回复</button></td>';
				}
			});
